...
class PriorityQueue{
    class Node{
    public:
        Node(T element, int8_t priority = 255);
        T value;
        Node* next;
...
public:
    PriorityQueue();
    void enqueue(T element, int8_t priority);
    T dequeue();
    T peek();
    bool isEmpty();
    void printQueue();
...
template <class T, Order order>
PriorityQueue<T, order>::Node::Node(T element, int8_t priority){
    this->value = element;
    this->priority = priority;
}

template <class T, Order order>
void PriorityQueue<T, order>::enqueue(T element, int8_t priority){
...
void PriorityQueue<T, order>::printQueue(){
    ...
    while(ptr != nullptr){
        std::cout <<"value: "<< ptr->value << "\tpriority: "<< (int)(ptr->priority) << std::endl;
